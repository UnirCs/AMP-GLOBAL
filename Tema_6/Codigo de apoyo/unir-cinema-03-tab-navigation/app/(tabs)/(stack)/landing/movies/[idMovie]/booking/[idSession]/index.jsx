...
export default function BookingSessionScreen() {
    ...
    const { movies, addTicket } = useMoviesContext();
    ...
    const handlePurchase = () => {
        ...
        // Crear el objeto ticket con la información de la compra
        const ticket = {
            movieTitle: movie.title,
            date: dateString,
            time: session.time,
            room: `Sala ${session.room || 1}`,
            seats: selectedSeats,
            totalPrice: totalPrice,
            purchaseDate: new Date().toISOString()
        };

        // Guardar el ticket en el contexto
        addTicket(ticket);

        // Mostrar alerta de confirmación
        Alert.alert(
            '¡Entradas compradas correctamente!',
            `Has comprado ${selectedSeats.length} entrada(s) para ${movie.title}`,
            [
                {
                    text: 'OK',
                    onPress: () => {
                        ...
                        router.dismissAll();
                        router.replace('/(tabs)/(stack)/landing');
                    }
                }
            ]
        );
